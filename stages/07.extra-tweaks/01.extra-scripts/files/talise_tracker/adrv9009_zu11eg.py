...
class adrv9009_zu11eg(adrv9009):
    ...
    def __init__(self, uri: str = "", jesd_monitor: bool = False, jesd=None) -> None:
        ...
        self.num_elements: int = 4
        self.pcal_data: list[float] = [0.0 for i in range(0, (self.num_elements - 1))]
        self.gcal_data: list[float] = [1.0 for i in range(0, self.num_elements)]

    def mcs_chips(self) -> None:
    ...
    @property
    def frequency_hopping_mode_chip_b(self) -> str:
    ...
    def frequency_hopping_mode_chip_b(self, value: str) -> None:
        self._set_iio_attr(
            "TRX_LO", "frequency_hopping_mode", True, value, self._ctrl_b
        )

    @property
    def frequency_hopping_mode_en_chip_b(self) -> str:
    ...
    def frequency_hopping_mode_en_chip_b(self, value: str) -> None:
        self._set_iio_attr(
            "TRX_LO", "frequency_hopping_mode_enable", True, value, self._ctrl_b
        )

    @property
    def calibrate_rx_phase_correction_en_chip_b(self) -> str:
    ...
    def calibrate_rx_phase_correction_en_chip_b(self, value: int) -> None:
        self._set_iio_dev_attr_str(
            "calibrate_rx_phase_correction_en", value, self._ctrl_b
        )

    @property
    def calibrate_rx_qec_en_chip_b(self) -> str:
    ...
    def calibrate_rx_qec_en_chip_b(self, value: int) -> None:
        self._set_iio_dev_attr_str("calibrate_rx_qec_en", value, self._ctrl_b)

    @property
    def calibrate_tx_qec_en_chip_b(self) -> str:
    ...
    def gain_control_mode_chan0_chip_b(self, value: str) -> None:
        self._set_iio_attr("voltage0", "gain_control_mode", False, value, self._ctrl_b)

    @property
    def gain_control_mode_chan1_chip_b(self) -> str:
    ...
    def gain_control_mode_chan1_chip_b(self, value: str) -> None:
        self._set_iio_attr("voltage1", "gain_control_mode", False, value, self._ctrl_b)

    @property
    def rx_hardwaregain_chan0_chip_b(self) -> float:
    ...
    def tx_hardwaregain_chan0_chip_b(self, value: float) -> None:
        self._set_iio_attr("voltage0", "hardwaregain", True, value, self._ctrl_b)

    @property
    def tx_hardwaregain_chan1_chip_b(self) -> float:
    ...
    def tx_hardwaregain_chan1_chip_b(self, value: float) -> None:
        self._set_iio_attr("voltage1", "hardwaregain", True, value, self._ctrl_b)

    @property
    def rx_rf_bandwidth_chip_b(self) -> int:
    ...
    @property
    def tx_rf_bandwidth_chip_b(self) -> int:
    ...
    @property
    def rx_sample_rate_chip_b(self) -> int:
    ...
    @property
    def tx_sample_rate_chip_b(self) -> int:
    ...
    def trx_lo_chip_b(self, value: int) -> None:
        self._set_iio_attr("altvoltage0", "frequency", True, value, self._ctrl_b)
    
    @property
    def gcal(self) -> list[float]:
        """gcal: linear gain coefficients for each channel [gain_ch0, gain_ch1, gain_ch2, gain_ch3]"""
        return self.gcal_data

    @gcal.setter
    def gcal(self, values: list[float]) -> None:
    ...
    def save_gain_cal(self, filename: str = "gain_cal_val.pkl") -> None:
        """ Saves gain calibration file."""
        with open(filename, "wb") as file:
            pickle.dump(self.gcal, file)

    def load_gain_cal(self, filename: str = "gain_cal_val.pkl") -> None:
        ...
        try:
            with open(filename, "rb") as file:
                self.gcal = pickle.load(file)
            logger.info("Gain calibration values loaded from file.")
        except FileNotFoundError:
            logger.info("file not found, loading default (no gain correction)")
            self.gcal = [1.0] * (self.num_elements)

    @property
    def pcal(self) -> list[float]:
        """pcal: phase differences in degrees [(rx0 - rx1) (rx0 - rx2) (rx0 - rx3)]"""
        return self.pcal_data
    
    @pcal.setter
    def pcal(self, values: list[float]) -> None:
        if isinstance(values, list) and all(isinstance(item, float) for item in values):
    ...
    def save_phase_cal(self, filename: str = "phase_cal_val.pkl") -> None:
        """ Saves phase calibration file."""
        with open(filename, "wb") as file:
            pickle.dump(self.pcal, file)

    def load_phase_cal(self, filename: str = "phase_cal_val.pkl") -> None:
        ...
        try:
            ...
            logger.info("Phase calibration values loaded from file.")
        except FileNotFoundError:
            logger.info("file not found, loading default (no phase shift)")
